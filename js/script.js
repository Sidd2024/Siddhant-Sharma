document.addEventListener("DOMContentLoaded", function() {
	const container = document.querySelector('.portrait-image');
	const saffronShades = ['#f2ca34','#FFD700','#fad065','#f9c544','#ff9933', '#f96f34'];
  
	let index = 0;
  
	function changeBackgroundColor() {
	  container.style.backgroundColor = saffronShades[index];
	  index = (index + 1) % saffronShades.length;
	  setTimeout(changeBackgroundColor, 1500); 
	}
  
	changeBackgroundColor();

	document.getElementById('getResume').addEventListener('click', ()=>{
		window.open('img/Siddhant Sharma Resume.pdf','_blank');
	})
  });
